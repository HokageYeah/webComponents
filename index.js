...
(function (factory) {
  ...
  function getScale(ele) {
    ...
    const scalex = Number(
      (ele.offsetWidth === undefined ? 1 : ele.offsetWidth / pos.width).toFixed(
        1
      )
    );
    const scaley = Number(
  ...
  function addOverlay({
    ...
    addRule(overlay.paddingTop, {
      ...
      left: `${
        elementInfo["margin-left"] + elementInfo["border-left-width"]
      }px`,
    });
    addRule(overlay.paddingRight, {
      ...
      right: `${
        elementInfo["margin-right"] + elementInfo["border-right-width"]
      }px`,
    });
    addRule(overlay.paddingBottom, {
      ...
      bottom: `${
        elementInfo["margin-bottom"] + elementInfo["border-bottom-width"]
      }px`,
      right: `${
        elementInfo["padding-right"] +
        elementInfo["margin-right"] +
    ...
    $(".size", overlay.tips).innerHTML = `${marginLevel.width / scalexE}x${
    ...
    if (elementInfo.top / scaleyE >= 24 + 8) {
      ...
      tipsTop =
        ((marginLevel.height + elementInfo.top) / scaleyE + 8) * scaleyP;
      tipsOrigin[0] = "top";
    }
    const lr = {
      left: "auto",
    ...
    if (
      (elementInfo.left / scalexE) * scalexP >
      document.body.clientWidth * 0.7
    ) {
      overlay.tips.classList.add("reverse-r");
      tipsOrigin[1] = "right";
      lr.right = `${
        document.body.clientWidth - (elementInfo.right / scalexE) * scalexP
      }px`;
    } else {
    ...
    addRule(
      overlay.tips,
      Object.assign(Object.assign({ top: `${tipsTop}px` }, lr), {
        display: "block",
        "z-index": maxZIndex,
      })
    );
  ...
  class DomInspector {
    ...
    enable(mode = "single") {
      ...
      if (this.env === "mobile") {
        body &&
          body.addEventListener("touchend", this._onMoveEnd, {
            capture: true,
            passive: false,
          });
        touchAction(false);
    ...
    pause() {
      ...
      if (this.env === "mobile") {
        body &&
          body.removeEventListener("touchend", this._onMoveEnd, {
            capture: true,
            // passive: false,
          });
        touchAction(true);
